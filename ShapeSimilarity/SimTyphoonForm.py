...
def typhoonForm(typhoonName):
    ...
    path_main_TYPHOON = 'C:/Users/zmyzq/Desktop/台风/CMABSTdata/CH2016BST.txt'

    source = open(path_main_TYPHOON, 'r')
    ...
    for i in range(len(Typhoon_coord_lat)):
        rectangular_coordinates_X_use.append(rectangular_coordinates_X[i][0:9])
        rectangular_coordinates_Y_use.append(rectangular_coordinates_Y[i][0:9])
    ...
    # indexx = Typhoon_record_time.index(1997081900)
    index = Typhoon_Name.index(typhoonName)  # 'Maysak'
    target_X = rectangular_coordinates_X_use[index]
    target_Y = rectangular_coordinates_Y_use[index]
    ...
    for i in range(N):
        ...
        Sim_distance[Typhoon_Name[i].strip()] = [C_ij[i], Typhoon_record_time[i], S_ij[i], D_ij[i]]
